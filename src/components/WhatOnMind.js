  ...
  return (
    <div>
      <div className="flex justify-between">
        <h1 className="font-bold text-xs md:text-lg lg:text-xl">{title}</h1>
        <div>
          <button
            className="md:px-1 sm:px-1 px-0 hover:bg-sky-100 "
            onClick={prevSlide}
          >
            ⬅
          </button>
          <button
            className="md:px-1 sm:px-1 px-0 hover:bg-slate-100 "
            onClick={nextSlide}
          >
            ➡
          </button>
      ...
      <div
        className="flex w-[1500px] md:w-[2000px] lg:w-[2700px] transition ease-out duration-40"
        ...
        {info?.map((res, index) => (
          <div key={res.id}>
            ...
            <img
              ...
              src={COROUSEL_IMG_ID + res.imageId}
            />
          </div>
        ))}
      </div>
